...
//		reg.addAttribute("android", "geolo_createUser_android");
	...
	public void sendMsg(String chatName, String msg, int chatType) throws Exception {
		...
		if (msg.isEmpty()) {
			Tool.initToast(ContextUtil.ctx, "sbx");
		}
		else {
			if (chatType == ChatItem.CHAT) {
				ChatManager cm = XmppConnection.getInstance().getConnection().getChatManager();
	...
	public void sendimgMsg(String msg, int chatType, int iswork) throws Exception {
		if (getConnection() == null){
			throw new Exception("XmppException");
		}
		org.jivesoftware.smack.packet.Message message = new org.jivesoftware.smack.packet.Message();
...
//			muc.addMessageListener(new TaxiMultiListener());
			...
			Tool.initToast(ContextUtil.ctx, "网络不给力,请重试");
				...
				try {
					...
					for (final RosterEntry entry : entries) {
						...
						if(entry.getType() == RosterPacket.ItemType.none){
							XmppConnection.getInstance().removeUser(entry.getUser());
							MsgDbHelper.getInstance(ContextUtil.ctx).delChatMsg(entry.getUser().split("@")[0]);
							NewMsgDbHelper.getInstance(ContextUtil.ctx).delNewMsg(entry.getUser().split("@")[0]);
							continue;
						}
