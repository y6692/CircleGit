...
public class ProgressView extends View {
	...
	private float countRecorderTime = 10000;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!linkedList.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				// ????????
				canvas.drawRect(left, 0, countWidth, getMeasuredHeight(),
						progressPaint);
				// ??????
				canvas.drawRect(countWidth, 0, countWidth + threeWidth,
						getMeasuredHeight(), breakPaint);
			...
			if (linkedList.getLast() <= 3000)
				canvas.drawRect(perPixel * 3000, 0, perPixel * 3000
						+ threeWidth, getMeasuredHeight(), threePaint);
		} else
			// ????????????
			canvas.drawRect(perPixel * 3000, 0, perPixel * 3000 + threeWidth,
					getMeasuredHeight(), threePaint);// ?????????????
		...
		if (isVisible) {
			if (currentState == State.START)
				canvas.drawRect(countWidth + perProgress, 0, countWidth
						+ firstWidth + perProgress, getMeasuredHeight(),
						firstPaint);
			else
				canvas.drawRect(countWidth, 0, countWidth + firstWidth,
						getMeasuredHeight(), firstPaint);
		}
		// ?????????????????????
