...
public class VideoRecorderActivity extends BaseActivity implements OnClickListener, OnTouchListener {
	...
	private Button flashIcon = null, btnCancel, btnNext,
			btnSwitchCamera = null;
	private boolean bSwitchCamera = true;
	private Button btnRecoder = null;
	boolean nextEnabled = false;

	private volatile NewFFmpegFrameRecorder NewFFmpegFrameRecorder;

	private boolean isPreviewOn = false;
	private int currentResolution = Constants.RESOLUTION_HIGH_VALUE;
	private Camera mCamera;

	private int previewWidth = 480, screenWidth = 480;
	private int previewHeight = 800, screenHeight = 800;
	...
	private long recordingTime = 10000L;
	private long recordingMinimumTime = 8000L;
	private long recordingChangeTime = 3000L;
...
//	public native static int checkNeonFromJNI();
	...
	private void initLayout() {
		...
		btnCancel = (Button) findViewById(R.id.recorder_cancel);
		btnCancel.setOnClickListener(this);
		btnNext = (Button) findViewById(R.id.recorder_next);
		btnNext.setOnClickListener(this);
		flashIcon = (Button) findViewById(R.id.recorder_flashlight);
		btnSwitchCamera = (Button) findViewById(R.id.recorder_frontcamera);
		flashIcon.setOnClickListener(this);

		btnRecoder = (Button) findViewById(R.id.recorder_comp);
...
//					// FuncCore.showToast(NewFFmpegFrameRecorderActivity.this,
//					// "wfljxj");
//					finish();
//					return;
//				}
				...
				layoutParam2.topMargin = screenHeight;
				...
				btnSwitchCamera.setOnClickListener(VideoRecorderActivity.this);
	...
	private void initVideoRecorder() {
		...
		NewFFmpegFrameRecorder = new NewFFmpegFrameRecorder(strVideoPath, 480, 800, 1);
		NewFFmpegFrameRecorder.setFormat(recorderParameters
				.getVideoOutputFormat());
		NewFFmpegFrameRecorder.setSampleRate(recorderParameters
				.getAudioSamplingRate());
		NewFFmpegFrameRecorder.setFrameRate(recorderParameters
				.getVideoFrameRate());
		NewFFmpegFrameRecorder
				.setVideoCodec(recorderParameters.getVideoCodec());
		NewFFmpegFrameRecorder.setVideoQuality(recorderParameters
				.getVideoQuality());
		NewFFmpegFrameRecorder.setAudioQuality(recorderParameters
				.getVideoQuality());
		NewFFmpegFrameRecorder
				.setAudioCodec(recorderParameters.getAudioCodec());
		NewFFmpegFrameRecorder.setVideoBitrate(recorderParameters
				.getVideoBitrate());
		NewFFmpegFrameRecorder.setAudioBitrate(recorderParameters
				.getAudioBitrate());
	...
	public class AsyncStopRecording extends AsyncTask<Void, Integer, Void> {
		...
		protected void onPreExecute() {
			...
			creatingProgress = new Dialog(VideoRecorderActivity.this,
		...
		private void getFirstCapture(byte[] data) {
			...
			captureBitmapPath = Util
					.createImagePath(VideoRecorderActivity.this);
		...
		protected Void doInBackground(Void... params) {
			...
			isFinalizing = false;
			if (NewFFmpegFrameRecorder != null && recording) {
				recording = false;
				releaseResources();
			}
			publishProgress(100);
...
//		Util.showDialog(VideoRecorderActivity.this, "ts", "qdfq", 2,
//				new Handler() {
//					@Override
		...
		private void record(ShortBuffer shortBuffer) {
			try {
				synchronized (mAudioRecordLock) {
					if (NewFFmpegFrameRecorder != null) {
						this.mCount += shortBuffer.limit();
						NewFFmpegFrameRecorder.record(0,
			...
			synchronized (mVideoRecordLock) {
				if (recording && rec && lastSavedframe != null
					...
					try {
						...
						NewFFmpegFrameRecorder.setTimestamp(lastSavedframe
								.getTimeStamp());
						NewFFmpegFrameRecorder.record(yuvIplImage);
					} catch (com.googlecode.javacv.FrameRecorder.Exception e) {
						Log.i("recorder", "lzcw" + e.getMessage());
						e.printStackTrace();
					}
				}
...
//				if (NewFFmpegFrameRecorder != null) {
//					NewFFmpegFrameRecorder.setImageWidth(previewWidth);
//					NewFFmpegFrameRecorder.setImageHeight(previewHeight);
//				}
		...
		if (Build.VERSION.SDK_INT > Build.VERSION_CODES.FROYO) {
			mCamera.setDisplayOrientation(Util.determineDisplayOrientation(
					VideoRecorderActivity.this, defaultCameraId));
	...
	public void onClick(View v) {
		...
		} else if (v.getId() == R.id.recorder_next) {
	...
	private void releaseResources() {
		...
		try {
			if (NewFFmpegFrameRecorder != null) {
				NewFFmpegFrameRecorder.stop();
				NewFFmpegFrameRecorder.release();
			}
		} catch (com.googlecode.javacv.FrameRecorder.Exception e) {
			e.printStackTrace();
		}
